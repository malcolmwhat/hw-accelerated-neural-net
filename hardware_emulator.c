...
void begin_hardware_acceleration()
{
	...
	for (uint32_t ob = 0; ob < hw->tile_height_reg; ob += hw->m_o)
	{
		...
		for (uint32_t ib = 0; ib < hw->tile_width_reg; ib += hw->m_i)
		{
...
void compute_step()
{
	for (uint32_t o = 0; o < ctrl_sig->m_o; o++)
	{
		for (uint32_t i = 0; i < ctrl_sig->m_i; i++)
		{
